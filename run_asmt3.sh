#!/bin/bash
FABRIC_DIR=$HOME/Desktop/blockchain/fabric-samples/fabric-samples
CHANNEL=mychannel
echo $FABRIC_DIR
export PATH=$FABRIC_DIR/bin:$PATH
if [ "$1" != "1" ]
then 
	CHAINCODE_JAVASCRIPT_DIR=$FABRIC_DIR/asmt3-eng-auction/chaincode-javascript-1
else
	CHAINCODE_JAVASCRIPT_DIR=$FABRIC_DIR/asmt3-eng-auction/chaincode-javascript-2
fi
CHAINCODE_GO_DIR=$FABRIC_DIR/asmt3-eng-auction/chaincode-go
...
then
	...
	--peerAddresses ${org_peer_address[2]} \
	--tlsRootCertFiles ${org_tls_root_cert[2]} \
	-c "{\"function\":\"SubmitBid\",\"Args\":[\"$2\", \"asset1\"]}"
	...
	--peerAddresses ${org_peer_address[2]} \
	--tlsRootCertFiles ${org_tls_root_cert[2]} \
	-c "{\"function\":\"SubmitBid\",\"Args\":[\"$3\", \"asset1\"]}"
	else
	peer chaincode invoke -o $ORDERER_PEER_ADDRESS --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_TLS_ROOT_CERT \
	-C $CHANNEL -n $LABEL_NO_VERSION --peerAddresses ${org_peer_address[0]} \
	--tlsRootCertFiles ${org_tls_root_cert[0]} \
	...
	--peerAddresses ${org_peer_address[2]} \
	--tlsRootCertFiles ${org_tls_root_cert[2]} \
	-c "{\"function\":\"SubmitBid\",\"Args\":[\"$4\", \"asset1\"]}"
	fi
	...
	"{\"function\":\"SubmitBid\",\"Args\":[\"$2\", \"asset1\"]}"

	# Set the admin of Org 2
	sleep 5
	export CORE_PEER_TLS_ENABLED=true
	...
	--peerAddresses localhost:9051 --tlsRootCertFiles \
	${PWD}/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt -c \
	"{\"function\":\"SubmitBid\",\"Args\":[\"$3\", \"asset1\"]}"


	# Set the admin of Org 3
	sleep 5
	export CORE_PEER_TLS_ENABLED=true
	...
	--peerAddresses localhost:9051 --tlsRootCertFiles \
	${PWD}/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt -c \
	"{\"function\":\"SubmitBid\",\"Args\":[\"$4\", \"asset1\"]}"
